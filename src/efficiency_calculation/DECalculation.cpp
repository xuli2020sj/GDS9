...
void DECalculation::SetCellDetector() {
    ...
    //探测器
    GPoint PAl;
    PAl.SetPoint(PCollimator, LOfCollimator, 0, 0);//铝帽前端坐标
    ...
    GPoint PEnd;
    PEnd.SetPoint(PAl, 2 * (dL_CupGe + hOfCup + hOfCrystalDead) + LOfCrystal, 0, 0);//铝壳后端
    ...
    if (!Cell[no].Plane.empty()) Cell[no].Plane.clear();
    GPoint PCu;
    PCu.SetPoint(PAl, hOfCup + dL_CupGe + hOfCrystalDead + LOfCrystal - LOfCold * 0.5, 0, 0);
    ...
    if (!Cell[no].Plane.empty()) Cell[no].Plane.clear();
    GPoint PAl0;
    PAl0.SetPoint(PAl, 0.5 * (PEnd.x - PAl.x), 0, 0);//铝壳体素中心，不是前端中心
    ...
    //5是准直器
    GPoint PPb0;
    PPb0.SetPoint(PCollimator, 0.5 * (PEnd.x - PCollimator.x), 0, 0);
    if (!Cell[no].Plane.empty()) Cell[no].Plane.clear();
    ...
    if (IsShield) {
        if (!Cell[no].Plane.empty()) Cell[no].Plane.clear();
        GPoint PShield;
        PShield.SetPoint(PCollimator, 0.5 * LOfShield, 0, 0);
...
void DECalculation::LInCellCalculationDec(GPoint PA, GPoint PB) {
    ...
    for (int n = 0; n < NuCellDec; n++) {
        if (Cell[n].NoM == -999) LInCell[n] = 0;
            ...
            LInCell[n] = TempCCal.L_Cell(PA, PB2, Cell[n], L);
        if (n == 0 && LInCell[0] < 0.01) {
            LInCell[0] = 0;
            break;
        }
...
void DECalculation::CalculationOnePoint(int key, GPoint _PA, GPoint Dec) {
    ...
    if (key == 0)
        for (no = 0; no < MaxData; no++) SumCEGeo_0[no] = 0;
    else
        for (no = 0; no < MaxData; no++) SumCEGeo[no] = 0;
...
void DECalculation::DetectorGeo(int key, GPoint _PA, GPoint _PB) {
    int no;
    if (key == 0)
        for (no = 0; no < MaxData; no++) CEGeo_0[no] = 0;
    else
        for (no = 0; no < MaxData; no++) CEGeo[no] = 0;
