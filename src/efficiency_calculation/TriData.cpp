...
TriDNodeDefine *TriData::MoveNode(TriDNodeDefine *node, int direction) {
    if (!node) return NULL;
        ...
        if (node->NextX && node->NoX < node->NextX->NoX) return node->NextX;//????1?x????????????;
        else
            return NULL;
    } else if (direction == -1) {
        if (node->LastX && node->NoX > node->LastX->NoX) return node->LastX;//????-1?x????????????;
        else
            return NULL;
    } else if (direction == 2) {
        if (node->NextY && node->NoY < node->NextY->NoY) return node->NextY;//????2?y????????????;
        else
            return NULL;
    } else if (direction == -2) {
        if (node->LastY && node->NoY > node->LastY->NoY) return node->LastY;//????-2?y????????????;
        else
            return NULL;
    } else if (direction == 3) {
        if (node->NextZ && node->NoZ < node->NextZ->NoZ) return node->NextZ;//????3?z????????????;
        else
            return NULL;
    } else if (direction == -3) {
        if (node->LastZ && node->NoZ > node->LastZ->NoZ) return node->LastZ;//????-3?z????????????;
        else
            return NULL;
    } else
        return NULL;
...
//????????????????node1???node2,direction=1 x????2 y????3 z????;
...
//???§ß???????????node1??????????node2???????,direction=1 x????2 y????3 z????;
...
TriDNodeHeadDefine *TriData::CreatNodeHead(int NuData) {
    ...
    for (int i = 0; i < NuData; i++) {
        nodehead->VARIABLESData.push_back(i);//?????????????????????????????????????????double????;
                                             //		CTemp.Format(_T("VARIABLE%d"),i);
        CTemp = "VARIABLE" + i;
        nodehead->VARIABLES.push_back(CTemp);//?????????????????????;
    }
...
void TriData::CopyHead(TriDNodeHeadDefine *nodehead1, TriDNodeHeadDefine *nodehead2) {
    nodehead2->DimensionOfNodes = nodehead1->DimensionOfNodes;//???????????????????????????????????1???????;
    nodehead2->NuNodesInX = nodehead1->NuNodesInX;            //x????????????????;
    nodehead2->NuNodesInY = nodehead1->NuNodesInY;            //y????????????????;
    nodehead2->NuNodesInZ = nodehead1->NuNodesInZ;            //z????????????????;
    nodehead2->NuNodesTotal = nodehead1->NuNodesTotal;        //??????????????;
    nodehead2->NuNodesUsed = nodehead1->NuNodesUsed;          //??????????????????;
    nodehead2->Title = nodehead1->Title;                      //?????????????????;
    ...
    nodehead2->NuDataUsed = nodehead1->NuDataUsed;    //?????????????????;
    nodehead2->Inf = nodehead1->Inf;                  //?????????;
    nodehead2->IsScientific = nodehead1->IsScientific;//?????????????????????????????????????;
    nodehead2->NuDecimal = nodehead1->NuDecimal;      //??????????§³????¦Ë??????????8¦Ë;
}
//??head1?????head2;
        ...
        for (j = 0; j < NuY; j++) {
            for (k = 0; k < NuX; k++) {
                ...
                if (k == 0) nodeLine2 = node2;
                    ...
                    ConnectTwoNodes(node1, node2, 1);
                node1 = node2;
            }
            if (j == 0) nodeFace2 = nodeLine2;
            else
                ConnectTwoLineNodes(nodeLine1, nodeLine2, 1, 2);
            nodeLine1 = nodeLine2;
        }
...
void TriData::outputdata_3d(TriDNodeHeadDefine *nodehead, std::string FullName, std::string Direction) {
    ...
    if (Direction.find("XYZ") >= 0 || Direction.find(("xyz")) >= 0) {
    ...
    else if (Direction.find("XZY") >= 0 || Direction.find(("xzy")) >= 0) {
        Direction1 = 1;
        Direction2 = 3;
        Direction3 = 2;
        ...
        std::cerr << " Direction selection is wrong in 3D output.";
        return;
    }
...
void TriData::outputdata_3d_ZB(TriDNodeHeadDefine *nodehead, std::string FullName, std::string Direction) {
    ...
    if (Direction.find(("XYZ")) >= 0 || Direction.find(("xyz")) >= 0) {
        ...
        std::cerr << " Direction selection is wrong in 3D output." << std::endl;
        return;
    }
    ...
    if (nodehead->IsScientific) output_data.setf(ios::scientific);
        ...
        output_data.setf(ios::fixed, ios::floatfield);
    output_data.precision(nodehead->NuDecimal);
...
string TriData::Process1stLine(char linetext[LineChar_MaxLength]) {
    ...
    for (i = 0, j = 0; i < LineChar_MaxLength; i++) {
        if (linetext[i] == '"') key++;
        else if (key == 1 && linetext[i] != ' ') {
            chartemp[j++] = linetext[i];
            chartemp[j] = '\0';
        }
        if (key == 2) break;
    }
    ...
    for (i = 0, j = 0, jj = 0, k = 0; i < LineChar_MaxLength; i++) {
        if (linetext[i] == '"') key++;
            ...
            if ((linetext[i] >= '0' && linetext[i] <= '9') || linetext[i] == '.' || linetext[i] == '+' || linetext[i] == '-') {
                chartemp2[jj++] = linetext[i];
                chartemp2[jj] = '\0';
            }
        }
        if (key == 2) {
            VARIABLESTemp.push_back(chartemp);
            VARIABLESDataTemp.push_back(chartemp2);
            k++;
...
bool TriData::Process3rdLine(char linetext[LineChar_MaxLength]) {
    ...
    for (i = 0; i < 499 && i <= CTemp.length(); i++) {
        if (chartemp2[i] == 'I' && chartemp2[i + 1] == '=') {
            IData = i;
            IsExistI = DimensionIJK;
            DimensionIJK++;
        }
        if (chartemp2[i] == 'J' && chartemp2[i + 1] == '=') {
            JData = i;
            IsExistJ = DimensionIJK;
            DimensionIJK++;
        }
        if (chartemp2[i] == 'K' && chartemp2[i + 1] == '=') {
            KData = i;
            IsExistK = DimensionIJK;
            DimensionIJK++;
        }
    }
    if (DimensionIJK == 0) return false;
    if (DimensionIJK > 3) return false;
    NuIJK = DimensionIJK;//IJK????????;

    if (IData > 0) {
        for (i = 0; i < 32; i++)
            chartemp[i] = '\0';
        for (i = IData + 2, j = 0, key = 0; i < 499 && i <= CTemp.length(); i++) {
            if (chartemp2[i] >= '0' && chartemp2[i] <= '9') {
                chartemp[j++] = chartemp2[i];
                chartemp[j] = '\0';
            } else
                break;
        }
...
int TriData::ProcessDirection(double x, double y, double z, TriDNodeDefine *nodeold) {
    ...
    b = fabs(y - nodeold->y);
    c = fabs(z - nodeold->z);
    if (KeyDirection[0] == 0)//???KeyDirection0?????????
    {
    ...
    if (KeyDirection[1] == 0)//???KeyDirection1??KeyDirection2?????????
    {
...
TriDNodeHeadDefine *TriData::ReadData(std::string FullName) {
    ...
    while (read) {
        ...
        read.getline(buf, LineChar_MaxLength);//???§Ø?????;
        CTemp.clear();
        CTemp = buf;
        NuLines++;
        ...
        if (NuLines == 1) {
            ...
            TriDataUsingTemp->NuDataUsed = NuData - NuIJK;//???????????????????????IJK?????;

            for (i = NuIJK; i < (int) VARIABLESTemp.size(); i++) {
                TriDataUsingTemp->VARIABLES.push_back(VARIABLESTemp[i]);//??????????§Ú???;
            ...
            if (TriDataUsingTemp->NuDataUsed != NuData - NuIJK) {
                Success = false;
                break;
            }
            NuDataTotal++;
            ...
            if (NuDataTotal > 1) {
                ...
                if (keyDirection == 1) node1 = MoveNode(node1, KeyDirection[0]);
                    ...
                    node2 = MoveNode(node2, KeyDirection[1]);
                    node1 = node2;
                } else {
                    node3 = MoveNode(node3, KeyDirection[2]);
                    node1 = node3;
                    node2 = node3;
                }
            }
...
TriDNodeHeadDefine *TriData::ReadData(string FullName, string direction) {
    ...
    while (read) {///
        ...
        } else if (CTemp.find(("END")) >= 0 || CTemp.find(("end")) >= 0 || CTemp.find(("End")) >= 0)
            break;
        else {
    ...
    if (IsExponent) {
        for (i = 0; i < NYT; i++) {
            if (yAxis[i] <= 0) yAxis2[i] = log(MinData);
                ...
                yAxis2[i] = log(yAxis[i]);
        }
        key = SplineInterpolation(xAxis, yAxis2, NYT, x, y, Nyt);
        for (i = 0; i < Nyt; i++)
            y[i] = exp(y[i]);
    } else
        key = SplineInterpolation(xAxis, yAxis, NYT, x, y, Nyt);
    return key;
}
...
bool TriData::LineInterpolation(double xAxis[], double yAxis[], int NYT, double x[], double y[], int Nyt) {
    ...
    for (j = 0; j < Nyt; j++) {
        ...
        if (i < 0) y[j] = yAxis[0];
            ...
            y[j] = (yAxis[i + 1] - yAxis[i]) * (x[j] - xAxis[i]) / (xAxis[i + 1] - xAxis[i]) + yAxis[i];
    }
    return true;
}
...
double TriData::Interpolation(TriDNodeDefine *node0, int direction, int Start, int End, int noData, double xdata) {
    ...
    for (i = 0, j = 0; i <= End && node1; i++) {
        ...
        node1 = MoveNode(node1, direction);//????1??2??3????x??y??z???????????????;
    }
    //???????,????????x??????????y???????????????x???ï…????›¥????y??????????????y?????ÈÎ;
    double x2[1] = {xdata};
    double y2[1] = {0};
...
bool TriData::MLEM_Iteration(TriDNodeHeadDefine *headA, string DirectionA, TriDNodeHeadDefine *headB, string DirectionB, TriDNodeHeadDefine *headX, string DirectionX, double Resid[]) {
    ...
    std::string CTemp;
    if (DirectionA.find(("XY")) >= 0 || DirectionA.find(("xy")) >= 0 || DirectionA.find(("Xy")) >= 0 || DirectionA.find(("xY")) >= 0) {
        directionAI = 1;
        directionAJ = 2;
    } else if (DirectionA.find(("YX")) >= 0 || DirectionA.find(("yx")) >= 0 || DirectionA.find(("Yx")) >= 0 || DirectionA.find(("yX")) >= 0) {
        directionAI = 2;
        directionAJ = 1;
    } else if (DirectionA.find(("XZ")) >= 0 || DirectionA.find(("xz")) >= 0 || DirectionA.find(("Xz")) >= 0 || DirectionA.find(("xZ")) >= 0) {
        directionAI = 1;
        directionAJ = 3;
    } else if (DirectionA.find(("ZX")) >= 0 || DirectionA.find(("zx")) >= 0 || DirectionA.find(("Zx")) >= 0 || DirectionA.find(("zX")) >= 0) {
        directionAI = 3;
        directionAJ = 1;
    } else if (DirectionA.find(("YZ")) >= 0 || DirectionA.find(("yz")) >= 0 || DirectionA.find(("Yz")) >= 0 || DirectionA.find(("yZ")) >= 0) {
        directionAI = 2;
        directionAJ = 3;
    } else if (DirectionA.find(("ZY")) >= 0 || DirectionA.find(("zy")) >= 0 || DirectionA.find(("Zy")) >= 0 || DirectionA.find(("zY")) >= 0) {
        directionAI = 3;
        directionAJ = 2;
    } else
        return false;
    if (DirectionB.find(("X")) >= 0 || DirectionB.find(("x")) >= 0) directionB = 1;
    else if (DirectionB.find(("Y")) >= 0 || DirectionB.find(("y")) >= 0)
        directionB = 2;
    else if (DirectionB.find(("Z")) >= 0 || DirectionB.find(("z")) >= 0)
        directionB = 3;
    else
        return false;
    if (DirectionX.find("X") >= 0 || DirectionX.find(("x")) >= 0) directionX = 1;
    else if (DirectionX.find(("Y")) >= 0 || DirectionX.find(("y")) >= 0)
        directionX = 2;
    else if (DirectionX.find(("Z")) >= 0 || DirectionX.find(("z")) >= 0)
        directionX = 3;
    else
        return false;
