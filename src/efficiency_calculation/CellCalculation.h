...
public:
    ...
    int PlaneNu(void) { return Plane.size(); }
    ...
    void Cuboid(GPoint p0, double x1, double x2, double y1, double y2, double z1, double z2) {
        ...
        if (!PM.empty()) PM.clear();
        P0.SetPoint(p0);
        GPlane px1('P', 1, 0, 0, x1);
        AddPlane(px1, '+');
        GPlane px2('P', 1, 0, 0, x2);
        AddPlane(px2, '-');
        GPlane py1('P', 0, 1, 0, y1);
        AddPlane(py1, '+');
        GPlane py2('P', 0, 1, 0, y2);
        AddPlane(py2, '-');
        GPlane pz1('P', 0, 0, 1, z1);
        AddPlane(pz1, '+');
        GPlane pz2('P', 0, 0, 1, z2);
        AddPlane(pz2, '-');
    }
    ...
    void Cuboid(GPoint p0, double dx, double dy, double dz) {
        ...
        if (!PM.empty()) PM.clear();
        P0.SetPoint(p0);
        GPlane px1('P', 1, 0, 0, p0.x - dx * 0.5);
        AddPlane(px1, '+');
        GPlane px2('P', 1, 0, 0, p0.x + dx * 0.5);
        AddPlane(px2, '-');
        GPlane py1('P', 0, 1, 0, p0.y - dy * 0.5);
        AddPlane(py1, '+');
        GPlane py2('P', 0, 1, 0, p0.y + dy * 0.5);
        AddPlane(py2, '-');
        GPlane pz1('P', 0, 0, 1, p0.z - dz * 0.5);
        AddPlane(pz1, '+');
        GPlane pz2('P', 0, 0, 1, p0.z + dz * 0.5);
        AddPlane(pz2, '-');
    }
    ...
    void Cylinder(GPoint p0, char axis, double radius, double height) {
        ...
        P0.SetPoint(p0);
        if (axis == 'x' || axis == 'X') {
            GPlane px1('P', 1, 0, 0, p0.x - height * 0.5);
            AddPlane(px1, '+');
            GPlane px2('P', 1, 0, 0, p0.x + height * 0.5);
            AddPlane(px2, '-');
        } else if (axis == 'y' || axis == 'Y') {
            GPlane py1('P', 0, 1, 0, p0.y - height * 0.5);
            AddPlane(py1, '+');
            GPlane py2('P', 0, 1, 0, p0.y + height * 0.5);
            AddPlane(py2, '-');
        } else if (axis == 'z' || axis == 'Z') {
            GPlane pz1('P', 0, 0, 1, p0.z - height * 0.5);
            AddPlane(pz1, '+');
            GPlane pz2('P', 0, 0, 1, p0.z + height * 0.5);
            AddPlane(pz2, '-');
        }
        GPlane pc(p0, axis, radius);
        AddPlane(pc, '-');
    }
    ...
    void Cylinder(GPoint p0, char axis, double radius1, double radius2, double height) {
        ...
        GPlane pc1(p0, axis, radius1);
        AddPlane(pc1, '+');
        GPlane pc2(p0, axis, radius2);
        AddPlane(pc2, '-');
    }
    //球,参数说明：体中心坐标，半径
    ...
    void Ball(GPoint p0, double radius1, double radius2) {
        ...
        if (!PM.empty()) PM.clear();
        P0.SetPoint(p0);
        GPlane ps1(p0, radius1);
        AddPlane(ps1, '+');
        GPlane ps2(p0, radius2);
        AddPlane(ps2, '-');
    }
    //设置物性参数
